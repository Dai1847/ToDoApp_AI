...
async function getStats(userId: string): Promise<{
    ...
    const [totalTasks, completedTasks, upcomingTasks, recentMemos] = await Promise.all([
        ...
        prisma.task.count({
            where: { userId, status: "completed", updatedAt: { gte: today } }
        }),
                    ...
                    <div className="space-y-4">
                        {stats.upcomingTasks.length > 0 ? (
                            stats.upcomingTasks.map((task) => (
                                <div
                                    key={task.id}
                                    className="glass-card p-4 flex items-center justify-between group hover:border-indigo-500/30 transition-colors"
                                >
                                    <div className="flex items-center gap-4">
                                        <div
                                            className={`p-2 rounded-lg ${task.priority === "high"
                                    ...
                                    <Link
                                        href={`/tasks/${task.id}`}
                                        className="opacity-0 group-hover:opacity-100 transition-opacity p-2 hover:bg-white/5 rounded-full"
                                    >
                ...
                <aside className="space-y-8">
                    ...
                    <section className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <Link
                                href="/tasks?view=daily"
                                className="glass-card p-4 text-center hover:bg-indigo-500/5 transition-colors"
                            >
                                <Calendar className="mx-auto mb-2 opacity-70" />
                                <span className="text-xs font-medium">日次</span>
                            </Link>
                            <Link
                                href="/tasks?view=weekly"
                                className="glass-card p-4 text-center hover:bg-indigo-500/5 transition-colors"
                            >
                        ...
                        <div className="space-y-3">
                            {stats.recentMemos.map((memo) => (
                                <Link
                                    key={memo.id}
                                    href={`/memos/${memo.id}`}
                                    className="glass-card p-4 block hover:bg-white/5 transition-colors"
                                >
